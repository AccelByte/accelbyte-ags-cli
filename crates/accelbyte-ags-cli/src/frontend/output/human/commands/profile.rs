...
fn render_profile_details(
    ...
    // Labels are the `ags config get/set` keys so the mapping is direct. The
    // last two rows aren't writable config keys, so they carry a trailing note:
    // `client-secret` is keychain-managed (read-only here) and `token` is
    ...
    let rows: Vec<(&str, String)> = vec![
        ...
        (
            ...
            format!(
                "{} (keychain, read-only)",
                presence_or_unset(config.has_secret)
            ),
        ),
        (
            "token",
            format!("{} (session)", presence_or_unset(config.has_token)),
        ),
    ];
