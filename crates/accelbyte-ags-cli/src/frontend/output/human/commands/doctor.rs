...
use ags_protocol::diagnostics::{CheckResult, CheckStatus, CheckTier, DoctorResult};
...
fn render_doctor_text(output: &DoctorResult) -> String {
    ...
    for (i, report) in output.reports.iter().enumerate() {
        ...
        // Headline goes last so it reads as the report's conclusion.
...
/// Append a heading and per-check lines for a single tier (e.g. "Config", "Auth", "Network") into the rendered output.
