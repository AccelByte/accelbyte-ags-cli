...
use ags_protocol::output::{FieldEntry, Section};
use ags_runtime::support::strings::pluralize;

use crate::frontend::style::span::{StyledLine, StyledSpan};
use crate::frontend::style::text::{SYMBOL_ERROR, SYMBOL_FIX, SYMBOL_INFO, SYMBOL_WARNING};
use crate::frontend::style::tone::Tone;

/// Display label for a config value's resolved source, shared by the human and
/// JSON config renderers so the two stay in lockstep.
pub(crate) fn config_source_label(source: &ConfigSource) -> String {
    match source {
        ...
        ConfigSource::Keychain => "keychain".to_string(),
