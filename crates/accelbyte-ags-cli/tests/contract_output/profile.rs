...
fn test_profile_show_human_has_labels() {
    ...
    // Labels are the `config get/set` keys so the mapping is direct; the last
    // two carry a note that they are not writable config keys.
    for label in [
        "base-url",
        "client-id",
        "namespace",
    ...
    assert!(
        stdout.contains("(keychain, read-only)"),
        "client-secret row must note it is keychain-managed, got: {stdout}"
    );
    assert!(
        stdout.contains("(session)"),
        "token row must note it is session state, got: {stdout}"
    );
}
