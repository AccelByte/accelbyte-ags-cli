use ags::frontend::output::human::templates::render_error_text as render_error;
use ags_runtime::runtime::dispatch::classify::classify_to_runtime_error;
use serde_json::json;

use crate::common::error_helpers::{error_detail, error_reason, error_suggestion_kind, error_tip};

/// A curated error code with an informative server message surfaces the server
/// message honestly under an operation-failed headline, keeping the code in the detail
#[test]
fn test_known_error_code_classifies_and_renders() {
    ...
    let error = classify_to_runtime_error(403, &body, "iam", "users", "get");
    ...
    // The honest headline is the operation, not the curated label
    assert!(
        rendered.contains("Get user failed"),
        "Should lead with the operation-failed headline: {rendered}"
    );
    // The raw server message is surfaced as the reason
    assert!(
        rendered.contains("insufficient permissions"),
        "Should surface the raw server message: {rendered}"
    );
    // The error code (with the curated label) stays in the detail
    assert!(
...
fn test_missing_error_code_field() {
    ...
    let error = classify_to_runtime_error(404, &body, "iam", "roles", "get");
    let rendered = render_error(
        &error.message,
        error_reason(&error),
        error_detail(&error),
...
/// A curated 401 code with an informative server message surfaces that message under
/// the operation-failed headline and drops the curated fix, deferring to the server
#[test]
fn test_classify_401_error() {
    ...
    assert_eq!(error_reason(&error), Some("unauthorized access"));
    assert_eq!(
        error_detail(&error),
        Some("Error code 20001 (Request was not authorized)")
    );
...
fn test_classify_501_not_implemented() {
    ...
    let error = classify_to_runtime_error(501, &body, "iam", "users", "get");
...
fn test_classify_400_with_details_pattern() {
    ...
    let error = classify_to_runtime_error(400, &body, "iam", "clients", "get");
...
/// Email conflict (10133) with an informative server message surfaces that message under
/// the operation-failed headline, annotates the code with the curated label, and drops the fix
#[test]
fn test_classify_mapped_10133_email_conflict() {
    ...
    let error = classify_to_runtime_error(409, &body, "iam", "users", "get");
    assert_eq!(error.message, "Get user failed");
    assert_eq!(error_reason(&error), Some("email already used"));
    assert_eq!(
        error_detail(&error),
        Some("Error code 10133 (Email address already in use)")
    );
    assert!(error.hint.is_none());
}
...
fn test_classify_mapped_10365_client_not_found() {
    ...
    let error = classify_to_runtime_error(404, &body, "iam", "clients", "get");
...
/// Password reuse (1015073) with an informative server message surfaces that message under
/// the operation-failed headline, annotates the code with the curated label, and drops the fix
#[test]
fn test_classify_mapped_1015073_password_reuse() {
    ...
    assert_eq!(
        error_reason(&error),
        Some("new password cannot same with old password")
    );
    assert_eq!(
        error_detail(&error),
        Some("Error code 1015073 (New password cannot be the same as the old password)")
    );
    assert!(error.hint.is_none());
}
