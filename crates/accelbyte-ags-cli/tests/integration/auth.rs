...
use ags_runtime::runtime::auth::tokens as service;
...
async fn test_authorization_code_exchange_sends_correct_request() {
    ...
    let client = reqwest::Client::new();
    let result = ags_runtime::runtime::auth::tokens::exchange_authorization_code(
        &client,
        &server.uri(),
        "test-client-id",
...
async fn test_authorization_code_exchange_handles_error() {
    ...
    let client = reqwest::Client::new();
    let error = ags_runtime::runtime::auth::tokens::exchange_authorization_code(
        &client,
        &server.uri(),
        "test-client-id",
        Some("test-client-secret"),
...
///
/// Uses `--grant client-credentials` so the auth flow proceeds past the
/// browser-interaction TTY check and actually exercises the
/// URL-parsing code path the regression originally hit.
#[test]
...
async fn test_client_credentials_self_heals_on_refresh_rejection() {
    ...
    use ags_runtime::runtime::config::ProfileConfig;
    ...
    struct TestSink;
    impl ags_protocol::event::ProgressSink for TestSink {
        /// Drop every event.
    ...
    let probed = ags_runtime::runtime::auth::operations::probe_existing_session(
        &client,
        profile,
        server.uri(),
    ...
    let outcome = ags_runtime::runtime::auth::operations::login_with_client_credentials(
        &client,
        ags_runtime::runtime::auth::operations::ClientCredentialsLogin {
    ...
    use ags_runtime::runtime::auth::operations::LoginOutcomeKind;
    assert!(
