use ags_runtime::runtime::config::{self, ProfileConfig};
use ags_runtime::runtime::execution::ResolutionInput;
...
async fn test_profile_namespace_isolation() {
    ...
    let staging_ctx =
        ags_runtime::runtime::execution::ExecutionContext::resolve(&staging_input, &http)
            .await
            .unwrap();
    ...
    let prod_ctx = ags_runtime::runtime::execution::ExecutionContext::resolve(&prod_input, &http)
        .await
        .unwrap();
