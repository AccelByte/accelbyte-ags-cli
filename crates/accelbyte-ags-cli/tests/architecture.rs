...
fn test_runtime_layer_has_no_user_facing_io() {
    ...
    for scanned_dir in SCANNED_DIRS {
        ...
        walk_rust_files(&path, &mut violations, &mut files_scanned);
    }
    assert!(
        files_scanned > 0,
...
fn walk_rust_files(dir: &Path, violations: &mut Vec<String>, files_scanned: &mut usize) {
    ...
    for entry in entries {
        ...
        let path = entry.path();
        if path.is_dir() {
            walk_rust_files(&path, violations, files_scanned);
