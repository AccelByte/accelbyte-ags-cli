...
/// Workflow detail envelope (`describe workflow <id>`) shape is stable.
/// The built-in `competitive-multiplayer` definition is static, so inputs and
/// steps are captured in full (not redacted) — drift in the `WorkflowInputView`
/// / `WorkflowStepView` fields is exactly what this guards. Only the volatile
/// CLI version is redacted.
#[test]
fn test_snapshot_workflow_detail() {
    let output = ags_isolated()
    ...
    assert!(output.status.success());
    let json: serde_json::Value = serde_json::from_slice(&output.stdout).unwrap();
    insta::assert_json_snapshot!("workflow_detail_envelope", {
        ".generated_by.version" => "[version]",
    }, &json);
}
