...
/// `--port` is accepted by the authorization-code flow. The browser flow can't
/// run in a non-interactive subprocess (assert_cmd pipes stdin/stderr), so it
/// stops at the "requires browser interaction" refusal — reaching that, rather
/// than a clap "unexpected argument" error, proves `--port` is a recognised
/// flag on `auth login`. The port's actual forwarding to the callback-server
/// binding needs a real browser/TTY and is exercised manually, not here.
#[test]
...
/// A fresh client-credentials login persists the grant type to the profile
/// config, so `profile show` and `config get grant-type` reflect how the
/// profile authenticated (regression: grant-type always showed "not set").
#[tokio::test]
#[serial_test::serial]
async fn test_client_credentials_login_persists_grant_type_to_profile_config() {
    let tmp = tempfile::tempdir().unwrap();
    let _home = TempEnvGuard::set("AGS_HOME", tmp.path().to_str().unwrap());
    let _no_kc = TempEnvGuard::set("AGS_NO_KEYCHAIN", "1");
    ...
    assert_eq!(
        ...
        "the login grant type must be persisted to the profile config"
    );
}
