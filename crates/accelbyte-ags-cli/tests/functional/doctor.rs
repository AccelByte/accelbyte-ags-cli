...
/// a missing stored token must produce a Warning, not a Fail, when client
/// credentials are present. Before the fix, Entry::new returning NoStorageAccess
/// propagated as a RuntimeError, causing the Auth tier to Fail and the Network
/// tier to be skipped with "earlier tier failed" even though auth was possible.
#[test]
