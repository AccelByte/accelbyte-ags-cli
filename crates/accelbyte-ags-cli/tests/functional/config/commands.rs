...
// ── Client secret (keychain-managed, read-only via config) ──

/// `config set client-secret` is rejected with guidance pointing at auth login,
/// since the secret lives in the OS keychain, not the config file.
#[test]
fn test_config_set_client_secret_is_rejected_with_guidance() {
    ags_isolated()
        .args(["config", "set", "client-secret", "shhh"])
        .assert()
        .failure()
        .stderr(predicate::str::contains("keychain"))
        .stderr(predicate::str::contains("auth login"))
        .stderr(predicate::str::contains("AGS_CLIENT_SECRET"));
}

/// `config unset client-secret` is rejected with the same keychain guidance.
#[test]
fn test_config_unset_client_secret_is_rejected_with_guidance() {
    ags_isolated()
        .args(["config", "unset", "client-secret"])
        .assert()
        .failure()
        .stderr(predicate::str::contains("keychain"))
        .stderr(predicate::str::contains("auth login"));
...
/// Get-all lists the client secret as a read-only entry (presence only, never
/// the value).
#[test]
fn test_config_get_all_shows_client_secret_read_only() {
    let tmp = tempfile::tempdir().unwrap();
    let home = tmp.path().to_str().unwrap();
...
/// Get of the client secret shows masked read-only presence, not the value.
#[test]
fn test_config_get_client_secret_shows_read_only_presence() {
    let tmp = tempfile::tempdir().unwrap();
    let home = tmp.path().to_str().unwrap();

    ags_isolated()
        .env("AGS_HOME", home)
        .args(["config", "get", "client-secret"])
        .assert()
        .success()
        .stdout(predicate::str::contains("read-only"));
}

/// Get of the client secret in JSON marks it read-only and never carries a value.
#[test]
fn test_config_get_client_secret_json_is_read_only() {
    let tmp = tempfile::tempdir().unwrap();
    let home = tmp.path().to_str().unwrap();

    let output = ags_isolated()
        .env("AGS_HOME", home)
        .args(["config", "get", "client-secret", "--format", "json"])
        .output()
        .unwrap();

    assert!(output.status.success());
    let json: serde_json::Value = serde_json::from_slice(&output.stdout).unwrap();
    assert_eq!(json["key"], "client-secret");
    assert_eq!(json["read_only"], true);
    assert!(
        json["value"].is_null(),
        "secret value must never be carried"
    );
}
