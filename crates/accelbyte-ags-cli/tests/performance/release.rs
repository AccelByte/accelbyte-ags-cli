...
fn test_release_render_list_100_items_under_50ms() {
    use ags::frontend::output::human::render;
    use ags::frontend::RenderOptions;
    use ags_protocol::output::{ApiBody, ApiOutput, CommandOutput};
    use ags_runtime::catalogue::Catalogue;
    use ags_runtime::runtime::dispatch::shape::shape_response;
    ...
    let resource = service
        ...
        .unwrap();
    // `list-admins` is a paginated list GET on iam/users (`{data, paging}`),
    // standing in for a generic user-list render. iam/users has no bare `list`.
    let op = resource
        .operations()
        .find(|operation| operation.name == "list-admins")
        .cloned()
        .unwrap();
    ...
    // The timing threshold assumes a release build; in a debug run the render
    // still executes (catching breakage) but the assertion is skipped.
