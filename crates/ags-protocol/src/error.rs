...
pub struct RuntimeError {
    ...
    pub trace: Option<Box<crate::output_views::ExecutionTrace>>,
}
...
impl RuntimeError {
    /// Construct a transport/network-layer error from a free-form message.
    /// Use this from any layer that produces network errors; the protocol
    /// crate does not depend on a specific HTTP client.
    ...
    /// Construct an `Internal` error with a free-text message. Used by runtime
    /// code that detects an invariant violation, a compile-time workflow error,
    /// or any condition not covered by the typed error categories.
    ...
    /// The trimmed, non-empty human-facing reason (`details.reason`), if any.
    /// Shared by the step-failure surfaces and the step summary so the rule for
    /// which field carries the reason, and how it is normalized, lives in one
