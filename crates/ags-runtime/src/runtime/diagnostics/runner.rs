...
use ags_protocol::diagnostics::{CheckResult, CheckStatus, CheckTier, DoctorReport, DoctorResult};
...
///
/// Short network-probe timeout so a hung environment fails fast rather than
/// blocking the doctor command. Differs intentionally from the default dispatch
/// client (`runtime::dispatch::http::build_http_client`, 60 s), which serves
/// real API calls and tolerates slower responses.
const NETWORK_PROBE_TIMEOUT_SECS: u64 = 10;

/// Build the short-timeout HTTP client used by the network reachability probe.
fn build_network_probe_client() -> Result<reqwest::Client, reqwest::Error> {
    reqwest::Client::builder()
        .user_agent(crate::runtime::dispatch::http::APPLICATION_USER_AGENT)
...
mod tests {
    ...
    /// The doctor network probe client sends the CLI application User-Agent header
    #[tokio::test]
    async fn test_network_probe_client_sends_application_user_agent() {
        let server = MockServer::start().await;
        Mock::given(method("GET"))
            .and(path("/iam/healthz"))
            .and(header(
                "user-agent",
        ...
        let client = build_network_probe_client().expect("probe client should build");
        let response = client
            .get(format!("{}/iam/healthz", server.uri()))
            .send()
            .await
