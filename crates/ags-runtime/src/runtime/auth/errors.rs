...
/// Headline for a missing-directory error.
...
pub enum AuthError {
    ...
    // --- Session expiry (probe path: the access token really was stale) ---
    ...
    #[error("Token refresh was rejected by the server")]
    RefreshRejected(String),
    #[error("No refresh token stored for this profile")]
    RefreshNoRefreshToken,
    #[error("The stored refresh token has expired")]
    RefreshTokenExpired,
...
fn build_auth_runtime_error(
    ...
    tip: Option<String>,
) -> ags_protocol::error::RuntimeError {
        ...
        details: if has_details {
            Some(Box::new(ErrorDetails {
                ...
                suggestion_kind: suggestion
                    .as_ref()
                    .map(|_| ags_protocol::error::SuggestionKind::Fix),
                tip,
            }))
        } else {
            None
...
impl From<AuthError> for ags_protocol::error::RuntimeError {
    fn from(e: AuthError) -> Self {
        ...
        match e {
            ...
            AuthError::RefreshRejected(reason) => build_auth_runtime_error(
                message,
                Some(reason),
                Some("Run 'ags auth login' to re-authenticate.".into()),
                None,
            ...
            AuthError::RefreshNoRefreshToken | AuthError::RefreshTokenExpired => {
            ...
            AuthError::AuthorizationFailed { description, .. } => build_auth_runtime_error(
                message,
                Some(description),
                // Structured suggestion line so the
                // user has a concrete next step after an IdP rejection.
                Some(
                    "Run `ags auth login` again, or contact your administrator if this persists."
                        .into(),
                ),
                None,
                None,
            ...
            AuthError::OAuthStateMismatch => build_auth_runtime_error(
                ...
                // Same structured next-step shape as
                // `AuthorizationFailed` — both surface to the same place.
                Some(
                    "Run `ags auth login` again, or contact your administrator if this persists."
                        .into(),
                ),
...
mod tests {
    ...
    fn test_keychain_operation_failed_runtime_message_matrix() {
        ...
        for (resource, operation, prefix) in cases {
            ...
            let runtime_err = ags_protocol::error::RuntimeError::from(auth_err);
    ...
    fn test_cannot_determine_dir_runtime_messages() {
        let secret_err = ags_protocol::error::RuntimeError::from(AuthError::CannotDetermineDir(
            AuthResource::ClientSecret,
        ));
        assert_eq!(secret_err.message, "Cannot determine secret file directory");

        let token_err = ags_protocol::error::RuntimeError::from(AuthError::CannotDetermineDir(
            AuthResource::Token,
        ));
