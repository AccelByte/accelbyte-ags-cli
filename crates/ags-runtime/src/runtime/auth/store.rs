...
///   (surfaces as `Platform secure storage failure: Unknown(38)`).
///
...
mod tests {
    ...
    /// Smoke-test that keychain_entry_opt returns Ok on the current platform.
    ///
    /// On systems with a keychain backend, Entry::new succeeds → Ok(Some(entry)).
    /// On systems without one (e.g. WSL2 without D-Bus), Entry::new returns
    /// NoStorageAccess → Ok(None). Either way the result must not be Err.
    ///
    /// Note: this test only proves the happy / no-backend path against the real
    /// keyring backend of the host machine. The specific NoStorageAccess → Ok(None)
    /// branch is covered end-to-end by the functional test
    /// `test_doctor_missing_token_with_client_creds_is_warning_not_fail` in
    /// `tests/functional/doctor.rs`, which runs `ags doctor` without
    /// AGS_NO_KEYCHAIN on a platform that may not have a keychain backend.
    #[test]
    fn test_keychain_entry_opt_does_not_err_on_unavailable_keychain() {
        let result = keychain_entry_opt("test-probe", "probe-user");
        assert!(
            result.is_ok(),
            "keychain_entry_opt must not surface NoStorageAccess as Err: {:?}",
            result
        );
    }
}
