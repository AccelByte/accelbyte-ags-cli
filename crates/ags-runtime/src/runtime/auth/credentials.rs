...
pub fn resolve_base_url_value(profile: &str) -> Option<String> {
    resolve_base_url(profile).map(|(url, _)| url)
}

/// Resolve the client ID for `profile`, dropping the source tag.
pub fn resolve_client_id_value(profile: &str) -> Option<String> {
...
pub async fn resolve_stored_client_secret(profile: &str) -> Option<String> {
