...
pub(crate) fn lookup_error(service: &str, error_code: i64) -> Option<ErrorMapping> {
    // The dispatch layer passes the CLI display name (e.g. "season-pass"), but the
    // per-service tables are keyed by the internal id ("seasonpass"). Resolve
    // display → internal first so hyphenated services still get curated messages.
    // Unknown names pass through unchanged and fall to the standard table below.
    let service = crate::catalogue::Catalogue::internal_name(service).unwrap_or(service);
...
mod tests {
    ...
    /// The dispatch layer passes the CLI DISPLAY service name (e.g. "season-pass"),
    /// but the per-service tables are keyed by the internal id ("seasonpass").
    /// `lookup_error` must resolve display → internal, or hyphenated services
    /// (seasonpass, cloudsave, gametelemetry) never get their curated messages.
    #[test]
    fn test_lookup_resolves_display_service_name_to_internal() {
        // 49141 lives in the seasonpass table.
        let internal = lookup_error("seasonpass", 49141).expect("internal id must map");
        assert_eq!(internal.message, "Tier item not found in the store");
        // The display name "season-pass" must resolve to the same table.
        let display = lookup_error("season-pass", 49141)
            .expect("display name 'season-pass' must resolve to the seasonpass table");
        assert_eq!(display.message, "Tier item not found in the store");
    }
}
