...
fn load_json_value(
    ...
    let object: serde_json::Value = serde_json::from_str(&data)
        .map_err(|e| internal_error(format!("Failed to parse {label}: {e}")))?;
    Ok(object.get(json_name).and_then(value_to_string))
}
...
fn update_json_file_field(
    ...
    if let Some(value) = value {
        object[json_name] = parse_json_value(value);
...
mod tests {
    ...
    fn test_profile_config_round_trip() {
        ...
        assert_eq!(
            loaded.grant_type,
            Some(ags_protocol::request::GrantType::AuthorizationCode),
        );
    }
