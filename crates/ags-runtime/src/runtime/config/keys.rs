...
/// CLI names that refer to the OAuth2 client secret. It is stored in the OS
/// keychain (never a plaintext config file), so it is not a `KNOWN_KEYS` entry
/// and cannot be written through `ags config set`.
pub const CLIENT_SECRET_ALIASES: &[&str] = &["client-secret", "secret"];

/// Whether `cli_name` refers to the keychain-managed client secret.
pub fn is_client_secret_key(cli_name: &str) -> bool {
    CLIENT_SECRET_ALIASES
        .iter()
        .any(|alias| alias.eq_ignore_ascii_case(cli_name))
}
...
mod tests {
    ...
    /// The client-secret aliases are recognised (case-insensitively) and are
    /// not writable config keys.
    #[test]
    fn test_is_client_secret_key_matches_aliases() {
        assert!(is_client_secret_key("client-secret"));
        assert!(is_client_secret_key("secret"));
        assert!(is_client_secret_key("CLIENT-SECRET"));
        assert!(!is_client_secret_key("client-id"));
        assert!(find_key("client-secret").is_none());
    }
}
