...
/// Nouns whose trailing `s` is not a plural marker — acronyms (`qos` =
/// Quality of Service) and proper nouns (`oculus`) — so the heuristic below
/// would wrongly strip it (`qos` → `qo`). Compared case-insensitively against
/// the whole identifier, including kebab-case resource names.
const UNCOUNTABLE: &[&str] = &["qos", "iap-oculus"];
...
pub fn derive_noun_from_method(method_name: &str, resource_name: &str) -> String {
...
mod tests {
    ...
    /// Acronyms and proper nouns whose trailing `s` is not a plural marker are
    /// left intact (regression: `qos` was singularized to `qo`).
    #[test]
    fn test_singularize_leaves_uncountable_nouns_intact() {
        assert_eq!(singularize("qos"), "qos");
        assert_eq!(singularize("QoS"), "QoS");
        assert_eq!(singularize("iap-oculus"), "iap-oculus");
    }
    ...
    /// A lone trailing ESC (no following sequence) is removed, not left behind
    /// or allowed to swallow a following character.
    #[test]
    fn test_strips_lone_trailing_escape() {
        assert_eq!(strip_terminal_control_sequences("tail\x1b"), "tail");
    }

    /// Multibyte UTF-8 between escapes survives stripping.
    #[test]
    fn test_preserves_unicode_between_escapes() {
        assert_eq!(
            strip_terminal_control_sequences("\x1b[32m✔\x1b[0m done"),
            "✔ done"
        );
    }
