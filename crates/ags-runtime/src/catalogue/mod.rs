...
use ags_protocol::catalogue::{BodySchema, OperationSchema, ServiceSchema};
use ags_protocol::error::RuntimeError;

pub use ags_protocol::catalogue::ServiceId;
pub use aliases::former_method_names;
...
impl Catalogue {
    ...
    /// Load a service schema into the in-memory cache on first miss, parsing it
    /// from the bundled spec directly — never the on-disk parsed-schema cache.
    ///
    /// For pure, offline contexts such as shell-completion generation, where
    /// touching the on-disk cache (and blocking on its file lock) is
    /// undesirable. Prefer [`Catalogue::get_or_load`] on runtime paths where the
    /// disk cache's parse-once speed matters.
    pub fn get_or_load_bundled(&mut self, service: &str) -> Result<&ServiceSchema, RuntimeError> {
        self.memory_cache.get_or_load_bundled(service)
    }
...
impl Catalogue {
    /// Insert a fully-parsed `ServiceSchema` directly into the memory cache.
    /// After insertion, `get_or_load(name)` returns this schema without
    /// consulting bundled specs or the on-disk cache.
    ///
    /// # Test use only
    ///
    /// This method is intended for integration tests that need to inject a
    /// minimal fake schema without hitting real specs. Do not call from
    /// production code paths.
    pub fn insert_for_tests(&mut self, service: &str, schema: ServiceSchema) {
        self.memory_cache.insert(service.to_string(), schema);
    }
}
