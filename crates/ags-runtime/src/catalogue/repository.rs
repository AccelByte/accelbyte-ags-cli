...
/// on `cache_lock_path()`) for the duration of this call. `write_to_cache`
/// creates its parent directory if missing, but does not itself enforce
/// mutual exclusion against other processes.
...
/// writes the resulting `ServiceSchema` to the platform cache directory on a
/// best-effort basis. Subsequent runs load directly from cache. Callers that
/// want to force a fresh parse should invoke `refresh_one` / `refresh_all` via
/// the `refresh-specs` subcommand instead.
///
/// Populating the disk cache is an optimisation: if the cache directory cannot
/// be locked or written (read-only, full, or — in parallel test runs —
/// concurrently removed), the freshly parsed schema is still returned. Only the
/// explicit `refresh_one` / `refresh_all` paths treat a cache-write failure as
/// an error, since an explicit rebuild genuinely cannot succeed without it.
pub fn load_service(service: &str) -> Result<(ServiceSchema, SpecSource), RuntimeError> {
    ...
    // Populate the disk cache as a best-effort optimisation. The parsed
    // `schema` is already in hand, so a lock or write failure must not fail
    // the load.
    if let Some(cached) = try_cache_parsed_schema(&cache_path, &schema) {
        return Ok((cached, SpecSource::Cache));
    }
    Ok((schema, SpecSource::Bundled))
}

/// Best-effort population of the on-disk spec cache for a freshly parsed schema.
///
/// Acquires the cache lock (so concurrent first-run invocations don't both
/// write), re-checks for an entry a competing process wrote while we were
/// parsing, and otherwise writes our copy. Returns `Some(schema)` only when a
/// competing writer populated the cache first; in every other case — including
/// any lock or write failure — returns `None`, and the caller falls back to the
/// schema it already parsed. Caching is an optimisation, so failure is
/// deliberately swallowed here rather than propagated.
fn try_cache_parsed_schema(cache_path: &Path, schema: &ServiceSchema) -> Option<ServiceSchema> {
    let lock_path = cache::cache_lock_path().ok()?;
    let _lock = FileLock::acquire(&lock_path, "spec cache").ok()?;
    if let Ok(Some(cached)) = cache::load_from_cache(cache_path) {
        return Some(cached);
    }
    let _ = cache::write_to_cache(cache_path, schema);
    None
}
...
mod tests {
    ...
    /// `write_to_cache` recreates the parent directory if it does not exist —
    /// covers the parallel-test race where a sibling tempdir is torn down
    /// between cache-lock acquisition and the write.
    #[test]
    fn test_write_to_cache_creates_missing_parent_directory() {
        let dir = tempdir().unwrap();
        let path = dir.path().join("missing").join("test.json");
        assert!(
            !path.parent().unwrap().exists(),
            "parent directory should not exist before write"
        );

        let spec = bundled::load_bundled_spec("iam").unwrap();
        let schema = crate::catalogue::parser::parse_spec("iam", &spec);
        cache::write_to_cache(&path, &schema).expect("write should create parent and succeed");
    ...
    /// A lazy `load_service` cache miss still returns the parsed schema when the
    /// cache directory cannot be created, locked, or written. Caching is a
    /// best-effort optimisation, never load-fatal — this is the root-cause
    /// guard for the parallel-test race where a sibling test tears down the
    /// `AGS_HOME` cache directory mid-write.
    #[test]
    #[serial_test::serial]
    #[cfg(unix)]
    fn test_load_service_succeeds_when_cache_dir_unwritable() {
        use std::os::unix::fs::PermissionsExt;

        // Clears `AGS_HOME` on drop so a panicking assertion below does not
        // leak env state into other serial tests in this process.
        struct HomeEnvGuard;
        ...
        // A read-only `AGS_HOME` means the `cache/` subdirectory cannot be
        // created, so both the cache-lock acquisition and the cache write fail.
        std::fs::set_permissions(tmp.path(), std::fs::Permissions::from_mode(0o555)).unwrap();
        std::env::set_var(config::ENV_HOME, tmp.path());

        let result = load_service("iam");

        // Restore write permission so the `TempDir` can be cleaned up, before
        // any assertion can panic and skip the restore.
        std::fs::set_permissions(tmp.path(), std::fs::Permissions::from_mode(0o755)).unwrap();

        let (schema, source) =
            result.expect("load must succeed even when the cache dir is unwritable");
        assert!(
            matches!(source, SpecSource::Bundled),
            "an unwritable cache forces the bundled-spec path"
        );
        assert!(
            !schema.resources.is_empty(),
            "the iam schema must still parse and carry resources"
        );
    }
}
