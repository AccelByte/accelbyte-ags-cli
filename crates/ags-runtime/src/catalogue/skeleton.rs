...
use ags_protocol::catalogue::{BodyField, BodySchema, OperationSchema};
...
pub(crate) fn build_body_schema_skeleton(schema: &BodySchema) -> Value {
    if schema.is_array {
        // Scalar array (`["a"]`): one placeholder of the element's type, no fields.
        if let Some(item_type) = &schema.item_type {
            return Value::Array(vec![scalar_placeholder_for_type(item_type, "item")]);
        }
        // Object array (`[{…}]`): one placeholder element built from the fields.
        return Value::Array(vec![build_fields_object(&schema.fields)]);
    }
...
fn field_to_value(field: &BodyField) -> Value {
    use ags_protocol::catalogue::BodyFieldType;
...
fn scalar_placeholder_for_type(
    field_type: &ags_protocol::catalogue::BodyFieldType,
    name: &str,
) -> Value {
    use ags_protocol::catalogue::BodyFieldType;
    match field_type {
...
mod tests {
    ...
    fn operation_with_body(fields: Vec<BodyField>) -> OperationSchema {
        OperationSchema {
            id: ags_protocol::catalogue::OperationId::new("TestOp"),
            name: "test".to_string(),
    ...
    /// Array-rooted bodies should produce a top-level array, not a bare object.
    #[test]
    fn test_array_rooted_body_produces_array_skeleton() {
        let fields = vec![make_field("slotId", BodyFieldType::String, true)];
        let operation = OperationSchema {
            id: ags_protocol::catalogue::OperationId::new("TestOp"),
            name: "test".to_string(),
            summary: "Test operation".to_string(),
        ...
        let array_value = value
            .as_array()
            .expect("array-rooted body must skeleton to a JSON array");
        assert_eq!(array_value.len(), 1);
        assert_eq!(array_value[0]["slotId"], "my-slot-id");
    }

    /// Scalar-array bodies skeleton to a top-level array of one bare placeholder,
    /// not an array containing an empty object.
    #[test]
    fn test_scalar_array_body_produces_array_of_scalars() {
        let operation = OperationSchema {
            id: ags_protocol::catalogue::OperationId::new("TestOp"),
            name: "test".to_string(),
            summary: "Test operation".to_string(),
        ...
        let array_value = value
            .as_array()
            .expect("scalar array body must skeleton to a JSON array");
        assert_eq!(array_value.len(), 1);
        assert!(
            array_value[0].is_string(),
            "scalar-array element must be a bare string, got {:?}",
            array_value[0]
        );
    }
