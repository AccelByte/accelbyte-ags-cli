...
use super::openapi::{SwaggerDefinition, SwaggerOperation, SwaggerProperty, SwaggerSpec};
...
pub fn parse_spec(service_name: &str, spec: &SwaggerSpec) -> ServiceSchema {
    ...
    for (path, methods) in &spec.paths {
        for http_method_name in &["get", "post", "put", "patch", "delete"] {
            ...
            let Some(parsed) = parse_operation(spec, path, http_method_name, operation) else {
                continue;
            };

            let key = (parsed.resource.clone(), parsed.method_name.clone());
            let scope_bucket = accumulator.entry(key).or_default();
            let version_bucket = scope_bucket.entry(parsed.scope.clone()).or_default();
            if let Some(existing) = version_bucket.insert(parsed.api_version, parsed.schema) {
                ...
                debug_assert!(
                    false,
                    "duplicate x-operationId contract at (resource={}, method={}, scope={}, version={}): \
                ...
                version_bucket.insert(parsed.api_version, existing);
            }
        }
    }
...
/// One operation parsed from the spec, ready to be slotted into the
/// `(resource, method) -> scope -> version` accumulator.
...
/// Parse a single non-deprecated OpenAPI operation into a `ParsedOperation`, or
/// `None` when it should be skipped: missing/empty `x-operationId`, a malformed
/// segment count or non-numeric version (asserted in debug), the `internal`
/// resource, or an unsupported HTTP method.
fn parse_operation(
    spec: &SwaggerSpec,
    path: &str,
    ...
    let x_operation_id = operation
        .x_operation_id
        .as_deref()
        .filter(|s| !s.is_empty())?;

    // Expect exactly 5 segments: service/scope/resource/version/method. Both
    // underflow and overflow (a stray slash) are spec bugs; without this check
    // `split` would silently fold the overflow into `parts[4]` and the
    // version-parse below would fail-and-skip, dropping the operation silently.
    ...
    // Parse the version integer from "v1", "v2", etc. A non-numeric segment
    // means the spec is malformed — assert in debug, skip in release so one bad
    // op doesn't poison the whole service. Coercing to 0 would create a ghost
    // contract selectable as `--api-version v0`.
    ...
    let http_method = parse_http_method(&http_method_upper)?;
    ...
    let description = (!operation.description.is_empty()).then(|| operation.description.clone());
...
/// Resolve the body schema for an operation by finding its body parameter's `schema`.
///
/// A body schema takes one of three shapes:
/// - Object body: the body parameter's `schema` has a top-level `$ref` to a named
///   definition (e.g. `{"$ref": "#/definitions/X"}`).
/// - Object array: `{"type": "array", "items": {"$ref": ...}}` — the `$ref` lives
///   one level down on `items`; the named definition supplies the element fields.
/// - Scalar array: `{"type": "array", "items": {"type": "string"}}` — an array of
///   primitives with no named fields; the element type is captured in `item_type`.
fn resolve_body_schema(spec: &SwaggerSpec, path: &str, http_method: &str) -> Option<BodySchema> {
    ...
    if let Some(ref_path) = schema_ref.ref_path.as_deref() {
        return resolve_object_body(spec, ref_short_name(ref_path), false);
    }

    // Anything else must be an explicit array to be a body we understand.
    if schema_ref.schema_type.as_deref() != Some("array") {
        return None;
    }
    let items = schema_ref.items.as_ref()?;

    // Object array: `items` is a `$ref` to a named definition.
    if let Some(ref_path) = items.ref_path.as_deref() {
        return resolve_object_body(spec, ref_short_name(ref_path), true);
    }

    // Scalar array: `items` is a primitive type with no fields to gather.
    // Anything that is not a recognised scalar (e.g. an inline-object array) is
    // left unmodelled and treated as bodyless, as before.
    //
    // `items` with neither `$ref` nor `type` (only extensions, or an undeclared
    // shape) returns `None` here and falls through to bodyless — the same safe
    // default as an unrecognised scalar below.
    let type_name = items.schema_type.as_deref()?;
    let item_type = scalar_type_from_swagger(type_name);
    if matches!(item_type, BodyFieldType::Object) {
        return None;
    }
    Some(BodySchema {
        definition_name: format!("array of {type_name}"),
        fields: vec![],
        is_array: true,
...
/// Build a `BodySchema` for a named-definition body (an object, or an array of
/// objects). Returns `None` when the definition has no declared properties.
fn resolve_object_body(spec: &SwaggerSpec, def_name: &str, is_array: bool) -> Option<BodySchema> {
    let definition = spec.definitions.get(def_name)?;
    let fields = resolve_definition_fields(spec, definition);
    // A definition with no declared properties is treated as bodyless.
    if fields.is_empty() {
        return None;
    }
    Some(BodySchema {
...
mod tests {
    ...
    /// Build a `SwaggerOperation` fixture with a `formData` file parameter.
    fn file_upload_operation(x_op_id: &str) -> SwaggerOperation {
    ...
    /// add-plus-remove that would leave a count unchanged. Update this list
    /// deliberately when a refresh changes the set.
    #[test]
    fn test_bundled_specs_file_upload_operations_are_locked() {
        use super::super::bundled::load_bundled_spec;
        use super::super::manifest::service_ids;
        let mut ids: Vec<String> = service_ids()
        ...
        ids.sort();
        let expected = [
            "achievement/admin/achievements/v1/import",
            "chat/admin/config/v1/import",
            "chat/admin/profanity/v1/import",
            "csm/admin/app-ui/v1/upload-assets",
            "lobby/admin/config/v1/import",
            "platform/admin/iap-apple/v1/upload-certificate",
            "platform/admin/iap-google/v1/upload-certificate",
            "platform/admin/iap-xbox/v1/upload-certificate",
            "platform/admin/key-groups/v1/upload-keys",
            "platform/admin/payment-config/v1/update-wxpay-certificate",
            "platform/admin/plugin-config/v1/upload-loot-box-certificate",
            "platform/admin/plugin-config/v1/upload-revocation-certificate",
            "platform/admin/plugin-config/v1/upload-section-certificate",
            "platform/admin/rewards/v1/import",
            "platform/admin/stores/v1/import-csv",
            "platform/admin/stores/v2/import",
            "session/admin/platform-credentials/v1/upload",
            "social/admin/stat-cycles/v1/import",
            "social/admin/stat-definitions/v1/import",
        ];
        assert_eq!(ids, expected);
    }

    /// Array-bodied operations should resolve to a body schema, not `None`.
    #[test]
    fn test_resolve_body_schema_handles_array_body() {
        use super::super::openapi::{SwaggerItemsRef, SwaggerParameter, SwaggerSchemaRef};

        let mut definitions = HashMap::new();
        definitions.insert(
            "apimodels.Item".to_string(),
            SwaggerDefinition {
                ...
                required: vec!["slotId".to_string()],
            },
        );

        let body_param = SwaggerParameter {
            name: "body".to_string(),
            ...
            format: String::new(),
            enum_values: None,
            schema: Some(SwaggerSchemaRef {
                ref_path: None,
                schema_type: Some("array".to_string()),
                items: Some(SwaggerItemsRef {
                    ref_path: Some("#/definitions/apimodels.Item".to_string()),
                    schema_type: None,
                }),
            }),
            default: None,
        ...
        paths.insert(
            "/inventory/v1/items/bulk".to_string(),
            HashMap::from([(
                "post".to_string(),
                SwaggerOperation {
                    is_deprecated: false,
                    ...
                    x_security: serde_json::Value::Null,
                    x_operation_id: Some("inventory/admin/items/v1/bulk-add".to_string()),
                    produces: Vec::new(),
                },
            )]),
        );
        ...
        let body = resolve_body_schema(&spec, "/inventory/v1/items/bulk", "POST")
            .expect("array body must resolve to Some, not None");
        assert!(
            body.is_array,
            "expected is_array: true for array-rooted body"
        );
        assert_eq!(body.definition_name, "apimodels.Item");
        assert_eq!(body.fields.len(), 1);
        assert_eq!(body.fields[0].name, "slotId");
        assert!(
            body.item_type.is_none(),
            "object array has no scalar item_type"
        );
    }

    /// Scalar-array bodies (`["string"]`) resolve to a fieldless body schema
    /// that carries the element type, so the command exposes `--json` instead of
    /// being treated as bodyless (e.g. IAM role assignment, entitlement revoke).
    #[test]
    fn test_resolve_body_schema_handles_scalar_array_body() {
        use super::super::openapi::{SwaggerItemsRef, SwaggerParameter, SwaggerSchemaRef};
        ...
        let mut paths = HashMap::new();
        paths.insert(
            "/iam/v3/admin/users/{userId}/roles".to_string(),
            HashMap::from([(
                "post".to_string(),
                SwaggerOperation {
                    is_deprecated: false,
                    ...
                    parameters: vec![body_param],
                    x_security: serde_json::Value::Null,
                    x_operation_id: Some("iam/admin/users/v3/add-roles".to_string()),
                    produces: Vec::new(),
                },
            )]),
        );
        ...
        let body = resolve_body_schema(&spec, "/iam/v3/admin/users/{userId}/roles", "POST")
            .expect("scalar array body must resolve to Some, not None");
        assert!(body.is_array, "scalar array body must set is_array");
        assert!(
            body.fields.is_empty(),
            "scalar array body has no named fields"
        );
        assert_eq!(body.item_type, Some(BodyFieldType::String));
    }
