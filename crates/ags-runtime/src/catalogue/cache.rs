...
///
/// Creates the parent directory if it does not yet exist. Callers also
/// acquire the cache lock via `FileLock::acquire`, which creates the
/// directory as a side-effect, but parallel test runs can tear the cache
/// dir down between acquire and write, so we re-create it defensively
/// here. The directory creation is idempotent.
