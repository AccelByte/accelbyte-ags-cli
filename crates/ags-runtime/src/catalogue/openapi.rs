...
//!
//! CLI-facing schemas live in `ags_protocol::catalogue`; this module only
...
pub struct SwaggerSchemaRef {
    ...
    #[serde(default)]
    pub items: Option<SwaggerItemsRef>,
}

/// The `items` shape of an array-typed body parameter schema.
///
/// Only the two fields the parser discriminates on are modelled: a `$ref`
/// (array of objects) or a primitive `type` (array of scalars). This is a
/// deliberately minimal type rather than reusing [`SwaggerProperty`], whose
/// object-property fields would always deserialise to defaults here and mislead
/// future readers into thinking they carry meaning.
#[derive(Debug, Deserialize)]
pub struct SwaggerItemsRef {
    #[serde(rename = "$ref", default)]
    pub ref_path: Option<String>,
    #[serde(rename = "type", default)]
    pub schema_type: Option<String>,
